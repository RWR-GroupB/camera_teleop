...
class HandTracker:
    ...
    def __init__(self, input_src=None,
                ...
                stats=False,
                trace=0,
                DEVICE_MXID = "18443010310CC71200"
                ):
        ...
        if DEVICE_MXID is None:
            ...
            if len(available_devices) > 1:
                print(f"Available devices: {available_devices}. Selecting the first one.")
            if len(available_devices) > 0:
                DEVICE_MXID = available_devices[0].getMxId()
            else:
                print("Error: No OAKD cameras found!")
